...
class smf2Extractor(ForumExtractor):
    class Thread(ItemExtractor):
        ...
        def get_contents(self,rq,url,t_id,**kwargs):
            ...
            if len(title) > 0:
                viewed = forumposts.search(r'div .cat_bar; h3 .catbg | "%i\n" / sed "s/<[^>]*>//g;s/ &nbsp;/ /;s/.* (\([^)]*\))$/\1/;s/.* \([0-9]*\) .*/\1/"')[:-1]
            else:
                title = forumposts.search('h1 | "%i\n", B>h[0-9] .display_title; span #top_subject | "%i"')
                viewed = forumposts.search('div .display-info;  li m@v>"comments" | "%i\n" / sed "s/<[^>]*>//g; s/ .*//"')[:-1]

            ret['title'] = title
            ret['viewed'] = viewed

            ret['path'] = json.loads(rq.search(r"""
                .path.a {
...
#ses = Session()
#x = smfExtractor(ses)
#x.get_thread('http://750mm.pl/index.php?topic=5559.0')
