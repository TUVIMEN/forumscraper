...
class xenforo2Extractor(ForumExtractor):
    class Thread(ItemExtractor):
        ...
        def get_reactions(self,rq,baseurl,first_delim,xfToken):
            ...
            if len(reactions_url) > 0:
                reactions_url = '{}{}{}_xfRequestUri=&_xfWithData=1&_xfToken={}&_xfResponseType=json'.format(baseurl,reactions_url,first_delim,xfToken)

                obj = reliq(self.session.get_json(reactions_url)['html']['content'].translate(str.maketrans('','','\n\t')))

                t = json.loads(obj.search(r"""
    ...
    class User(ItemExtractor):
        def __init__(self,session):
            ...
            self.match = [re.compile(r'https?://([a-zA-Z0-9-]+\.)+[a-zA-Z]+/(.*[./])?(\d+)/[\&?]tooltip=true\&.*'),3]
            self.path_format = 'm-{}'
    ...
    def __init__(self,session=None,**kwargs):
        ...
        self.board_forums_expr = reliq.expr(r'* .node-title; a href -href=b>/link-forums/ | "%(href)v\n"')
...
class xenforo1Extractor(ForumExtractor):
    class Thread(ItemExtractor):
        ...
        def get_contents(self,settings,rq,url,t_id):
            ...
            while True:
                for i in rq.filter(r'ol #messageList; li #E>post-[0-9]* data-author l@[1]').children():
                    post = {}
                    ...
                    if len(avatar) == 0:
                        user_id = messageUB.search(r'span class="img m" style | "%(style)v" / sed "s#^.*/avatars/m/[0-9]*/##; s#\..*##"')
                        avatar = messageUB.search(r"""span class="img m" style | "%(style)v" / sed "s/?[0-9]*')//; s/background-image: url('/\//; s/')$//" """)
                    else:
    ...
    def __init__(self,session=None,**kwargs):
        ...
        self.board_forums_expr = reliq.expr(r'h3 .nodeTitle; a -href=a>"#" href | "%(href)v\n"')
        self.forum_forums_expr = self.get_board
...
#ses = Session()
#x = xenforo1Extractor(ses)
#x.get_thread('https://www.forumhouse.ru/threads/409354/')
#ses = Session()
#x = xenforo2Extractor(ses)
#x.get_thread('https://xenforo.com/community/threads/can-not-search-chinese.2049/')
